...
test("restore prefers the remembered session when it still belongs to the workspace", () => {
  const sessions = [
    { id: "old", cwd: "/repos/a", modified: "2026-08-19T10:00:00.000Z", projectKey: "project:a" },
    { id: "new", cwd: "/repos/a", modified: "2026-08-20T10:00:00.000Z", projectKey: "project:a" },
  ];
  assert.equal(
    pickWorkspaceSessionToRestore(sessions, "project:a", "old")?.id,
    "old",
  );
});

test("restore falls back to the latest session when memory is missing or stale", () => {
  const sessions = [
    { id: "old", cwd: "/repos/a", modified: "2026-08-19T10:00:00.000Z", projectKey: "project:a" },
    { id: "new", cwd: "/repos/a", modified: "2026-08-20T10:00:00.000Z", projectKey: "project:a" },
    { id: "other", cwd: "/repos/b", modified: "2026-08-21T10:00:00.000Z", projectKey: "project:b" },
  ];
  assert.equal(pickWorkspaceSessionToRestore(sessions, "project:a", null)?.id, "new");
  assert.equal(pickWorkspaceSessionToRestore(sessions, "project:a", "deleted")?.id, "new");
});

test("restore matches a workspace even when the incoming key is the project root path", () => {
  const sessions = [
    {
      id: "latest",
      cwd: "/repos/a/worktrees/feat",
      projectRoot: "/repos/a",
      projectKey: "project:a",
      modified: "2026-08-20T12:00:00.000Z",
    },
  ];
  assert.equal(sessionBelongsToWorkspace(sessions[0], "/repos/a"), true);
  assert.equal(pickWorkspaceSessionToRestore(sessions, "/repos/a", null)?.id, "latest");
});

test("restore stays empty when the workspace has no sessions", () => {
  const sessions = [
    { id: "other", cwd: "/repos/b", modified: "2026-08-21T10:00:00.000Z", projectKey: "project:b" },
  ];
  assert.equal(pickWorkspaceSessionToRestore(sessions, "project:a", "old"), null);
  assert.equal(pickWorkspaceSessionToRestore([], "project:a", null), null);
});
